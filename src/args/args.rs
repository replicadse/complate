...
impl ClapArgumentLoader {
    pub async fn load_from_cli() -> std::io::Result<Command> {
        ...
        match command.subcommand_matches("init") {
            Some(_) => {
        ...
        match command.subcommand_matches("print") {
            Some(x) => {
                ...
                return Ok(Command::Print(PrintArguments {
                    configuration: config,
                    shell_trust: shell_trust,
                }));
            }
            None => {
                return Err(std::io::Error::new(std::io::ErrorKind::Other, "could not resolve subcommand", ));
            }
        }
    }
}
